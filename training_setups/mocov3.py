...
class MoCo(nn.Module):
    ...
    def __init__(
        ...
        if hasattr(self.encoder_q, "fc"):
            ...
            dim_mlp = self.encoder_q.classifier[-1].weight.shape[1]
            self.encoder_q.classifier[-1] = nn.Sequential(
                nn.Linear(dim_mlp, dim_mlp),
                nn.BatchNorm1d(dim_mlp),
                nn.ReLU(),
                nn.Linear(dim_mlp, dim_mlp),
                nn.ReLU(),
                self.encoder_q.classifier[-1],
            )
            self.encoder_k.classifier[-1] = nn.Sequential(
                nn.Linear(dim_mlp, dim_mlp),
                nn.BatchNorm1d(dim_mlp),
                nn.ReLU(),
                nn.Linear(dim_mlp, dim_mlp),
                nn.ReLU(),
                self.encoder_k.classifier[-1],
            )
